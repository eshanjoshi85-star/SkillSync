...
async function findMatches(currentUserId) {
    ...
    const currentUserSkills = await prisma_1.prisma.userSkill.findMany({
    ...
    const potentialTeachers = await prisma_1.prisma.userSkill.findMany({
        where: {
    ...
    if (canTeach.length > 0) {
        const mutualLearners = await prisma_1.prisma.userSkill.findMany({
            where: {
                skillId: { in: canTeach },
                type: "LEARN",
        ...
        const mutualUserIds = new Set(mutualLearners.map((s) => s.userId));
        for (const [uid, match] of matchMap) {
            if (mutualUserIds.has(uid))
